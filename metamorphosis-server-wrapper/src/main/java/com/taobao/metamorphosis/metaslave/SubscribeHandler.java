...
public class SubscribeHandler implements SubscribeHandlerMBean {
    ...
    public SubscribeHandler(final MetaMorphosisBroker broker) throws MetaClientException {
        ...
        this.slaveOffsetStorage = new SlaveOffsetStorage(this.broker, this.slaveZooKeeper, this.sessionFactory.getRemotingClient());
        this.messageListener = new MetaSlaveListener(this.broker.getBrokerZooKeeper(), this.broker.getStoreManager(),
    ...
    private void tryReloadConfig(BooleanCommand resp) throws UnsupportedEncodingException, IOException, FileNotFoundException {
    ...
    synchronized public void start() {
        ...
        try {
            ...
            final Map<String, List<Partition>> partitionsForTopics = this.slaveZooKeeper.getPartitionsForTopicsFromMaster();
    ...
    private void initMessageConsumer() throws Exception {
        if (this.consumer == null) {
            final ConsumerConfig consumerConfig = new ConsumerConfig(this.broker.getMetaConfig().getSlaveConfig().getSlaveGroup());
            consumerConfig.setMaxDelayFetchTimeInMills(this.broker.getMetaConfig().getSlaveConfig().getSlaveMaxDelayInMills());
            // 消息处理失败卡在那一条上,不进入recover
            consumerConfig.setMaxFetchRetries(Integer.MAX_VALUE);
