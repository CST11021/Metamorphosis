...
#### #生产者发布topic时，做了哪些事情，如果没有发布topic，生产者可以发送消息吗？

​		生产者发布topic首先是在zk上创建创建一个节点（/meta/brokers/topics-pub/${topic}），并监听该路径子节点的变更情况；然后获取topic对应的MQ服务器信息，生产者断开与所有旧的MQ服务器的连接，然后再重新连接到新的MQ集群；如果topic对应的MQ集群发生变更，则执行所有监听了该topic的监听器。
